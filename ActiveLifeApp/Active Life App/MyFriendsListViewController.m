...
{
    return search ? [_searchArr count]:[_responseArr count];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    labelName.text = [[search ?_searchArr : _responseArr objectAtIndex:indexPath.row] valueForKey:@"name"];
    [cell.contentView addSubview:labelName];
    
    NSTextAttachment *attachment = [[NSTextAttachment alloc] init];
    attachment.image = [UIImage imageNamed:@"location_icon_small.png"];
    NSAttributedString *attachmentString = [NSAttributedString attributedStringWithAttachment:attachment];
    NSMutableAttributedString *myString= [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@" %@",[[search ?_searchArr : _responseArr objectAtIndex:indexPath.row] valueForKey:@"country"]]];
    ...
    NSURL *url = [NSURL URLWithString:[[search ?_searchArr : _responseArr objectAtIndex:indexPath.row] valueForKey:@"profile_picture"]];
    ...
    UIButton *buttonAdd = [[UIButton alloc] initWithFrame:CGRectMake(260,14,35,35)];
    
    if (segmentIndex == 0) {
        [buttonAdd setImage:[UIImage imageNamed:@"checkmark_icon"]  forState:UIControlStateNormal];
    }
    else{
...
//    [buttonAdd setImage:[UIImage imageNamed:@"checkmark_icon"]  forState:UIControlStateNormal];
//    [buttonAdd setImage:[UIImage imageNamed:@"pending"]  forState:UIControlStateSelected];
    ...
    if ([[[search ?_searchArr : _responseArr objectAtIndex:indexPath.row] valueForKey:@"friendship_status"] isEqualToString:@"0"]) {
        [buttonAdd setImage:[UIImage imageNamed:@"pending"]  forState:UIControlStateNormal];
            buttonAdd.enabled = YES;
    }
    else   if ([[[search ?_searchArr : _responseArr objectAtIndex:indexPath.row] valueForKey:@"friendship_status"] isEqualToString:@"1"]) {
...
//        buttonAdd.selected = YES;
    ...
    UILabel *labelSeparator = [[UILabel alloc]initWithFrame:CGRectMake(10,63,290,1)];
...
//    [self fetchFriendsListForSearchText:searchStr andPage:1];
    if (segmentIndex == 0) {
        predicate = [NSPredicate predicateWithFormat:@"SELF.name beginswith[cd] %@",searchStr];
    }
    else{
        predicate = [NSPredicate predicateWithFormat:@"SELF.name beginswith[cd] %@",searchStr];
    }
